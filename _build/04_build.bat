...
rem USAGE:
rem   04_build.bat <build-type> <build-target> <cmake-cmdline>...
rem 
rem Description:
...
if "%CMAKE_BUILD_TYPE%" == "*" (
  for %%i in (%CMAKE_CONFIG_TYPES:;= %) do (
    ...
    call "%%CONTOOLS_ROOT%%/std/setshift.bat" -exe 2 CMAKE_CMD_LINE %%*
    call :BUILD || exit /b
  )
) else (
  call "%%CONTOOLS_ROOT%%/std/setshift.bat" -exe 2 CMAKE_CMD_LINE %%*
...
call "%%CONTOOLS_BUILD_TOOLS_ROOT%%/callln.bat" pushd "%%CMAKE_BUILD_DIR%%" || goto BUILD_END
