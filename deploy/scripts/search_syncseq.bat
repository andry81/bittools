...
rem call :CMD "%%~dp0bitsync.exe" /disable-calc-autocorr-mean /autocorr-mean-min 0.99 /s "%%STREAM_BYTE_SIZE%%" /q "%%SYNCSEQ_BIT_SIZE%%" /r "%%SYNCSEQ_REPEAT%%" /k "%%SYNCSEQ_INT32%%" sync "%%BITS_PER_BAUD%%" "%%FILE_IN%%" . && exit /b 0
...
for /F "usebackq eol= tokens=* delims=" %%i in (`dir /A:-D /B /O:N "%FILE_DIR_IN%%FILE_NAME_IN%.*%FILE_EXT_IN%"`) do (
  set "FILE_NAME=%%~ni"
  call :CMD "%%~dp0bitsync.exe" /disable-calc-autocorr-mean /autocorr-mean-min 0.99 /s "%%STREAM_BYTE_SIZE%%" /q "%%SYNCSEQ_BIT_SIZE%%" /r "%%SYNCSEQ_REPEAT%%" /k "%%SYNCSEQ_INT32%%" sync "%%BITS_PER_BAUD%%" "%%i" . ^
