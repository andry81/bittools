...
#include "help_inl.hpp"
                ...
                const tackle::path_tstring in_file_path = g_options.input_file;

                if (g_options.output_file_dir.empty()) {
                    g_options.output_file_dir = utility::get_parent_path(in_file_path);
                }
                ...
                case Mode_Gen:
                {
                    ...
                    gen_data.bits_per_baud = g_options.bits_per_baud;
                    gen_data.baud_mask = baud_mask;
                    gen_data.baud_capacity = baud_capacity;

                    gen_data.has_stream_byte_size = !!g_options.stream_byte_size;
                    ...
                    for (uint32_t j = 0; j < g_options.bits_per_baud; j++) {
                        ...
                        for (uint32_t i = 1; i < num_baud_alphabet_sequences; i++) {
                            ...
                            tackle::path_tstring out_file{ g_options.output_file_dir / utility::get_file_name_stem(in_file_path) + _T(".") +
                                utility::int_to_dec(j, 1, utility::tag_tstring{}) + _T("-") + utility::int_to_dec(i, 2, utility::tag_tstring{}) +
                                boost::fs::path{ in_file_path.str() }.extension().tstring() };
                ...
                case Mode_Sync:
                {
                    ...
                    sync_data.syncseq_bit_size = g_options.syncseq_bit_size;
                    sync_data.syncseq_int32 = g_options.syncseq_int32;
                    sync_data.syncseq_mask = syncseq_mask;
                    sync_data.syncseq_repeat = g_options.syncseq_repeat;
